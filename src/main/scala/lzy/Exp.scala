...
enum Exp:
  ...
  case L(idx: Int, body: Exp, env: Map[Int, Exp])
  case v(idx: Int)
  case If(cond: Exp, ok: Exp, no: Exp)

  lazy val evaluated: Exp = Evaluator.evaluate(this)
...
object Exp:
  def apply(tokens: Seq[Token]): Exp =
    tokens.foldRight(Seq.empty[Exp]) {
      ...
      case (Token.L(idx), body +: seq) => Exp.L(idx, body, Map.empty) +: seq
  ...
  def show(exp: Exp): String =
    exp match
      ...
      case Exp.L(idx, body, env) => "(\\v" + idx.toString + " -> " + show(body) + " [" + env.view.mapValues(Exp.show).mkString + "])"
